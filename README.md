...
- Maven
  ...
  3. 빌드, 패키징(jar/war파일 생성), 테스트 등을 한 번의 명령으로 간편하게 처리
...
- Gradle
  ...
  5. 빌드과정을 병렬처리, 캐싱사용으로 Maver보다 10~100배 향상된 성능 구현
...
- Gradle 오류 해결방법
  ...
  - Gralde JAVA Home -> 환경변수 JAVA_HOME의 경로 입력
...
- 각 폴더 구조
  ...
  - com.dongho98.spring03 : 패키지와 폴더로 구성
    - HelloController 클래스에 접근하려면
    - com.dongho98.spring03.controller.HelloController 접근해야 함
  ...
  - src/main/java : JUnit 스프링부트 테스트도구 자바파링 저장되는 위치
...
- Spring Boot JPA + Oracle + Thymeleaf + React
  ...
  - Thymeleaf - JSP 단점을 보안한 탬플릿 형태 FE 개발방식
    - Bootstrap은 필수로 사용
...
- Spring Boot 프로젝트 생성
  ...
  1. 명령 팔레트 시작(Ctrl + Shift + P) : Spring Initializer: Create a Gradle(Maven) Project
  2. Spring Boot version : 3.5.3
  3. Project language : Java
  4. Group Id : com.pknu(통일)
...
- Spring Boot 설정파일
  ...
  - application.properties - Spring Boot 프로젝트 자체 설정
  - setting.gradle, gradle-wrapper.properties - 손댈일 없음
...
1. 기본 실행
   ...
   1. resource/application.properties
      ...
      logging.file.name=D:/temp/backboard.log # 로그 파일 위치
      ```
   ...
   3. Controller 작업
      ...
      - MainController 생성
      - 새 파일로 생성 or Menu New file > class 둘 다 동일
...
2. DB연동
   ...
   1. H2 DB 의존성 추가
   2. applicaion.properties에 H2 관련 설정 추가
      ```properties
      ## H2 DB 설정
      spring.h2.console.enabled=true
      spring.h2.console.path=/h2-console // console뒤에 공간 있으면 안됨
...
2. DB연동 개발 계속
  ...
  5. Service 작성
    ...
    - Controller는 게이트(문) 사용자가 접속해서 요철하는 부분
